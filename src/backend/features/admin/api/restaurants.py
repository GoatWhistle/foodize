    ...
    for rid in body.ids:
        try:
            async with session.begin_nested():
                await moderation.moderate_restaurant(session, rid, ModerationStatus.APPROVED.value)
            ...
            failed.append({"id": str(rid), "error": "operation_failed"})
...
async def batch_reject_restaurants(
    ...
    for rid in body.ids:
        try:
            async with session.begin_nested():
                await moderation.moderate_restaurant(
