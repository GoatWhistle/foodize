...
async def _fetch_revenue_by_day(
    session: AsyncSession, order_filters: list
) -> dict[date, int]:
...
async def get_finance_analytics(
    ...
    prev_revenue = await _fetch_prev_revenue(session, prev_start, prev_end, vendor_id, restaurant_id)
