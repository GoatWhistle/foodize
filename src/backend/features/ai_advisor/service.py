...
from features.ai_advisor.tool_specs import build_advisor_tools
...
def to_messages(items: Iterable[ChatMessageIn]) -> list[Message]:
...
async def generate_insights(vendor: VendorProfile, language: str = DEFAULT_LANGUAGE) -> str:
    ...
    try:
        ...
        text, _ = await run_agent(
            ...
            system=build_system_prompt(language),
            messages=[
                Message(role=Role.USER, content=translate("prompts.advisor.insights", language))
            ],
            tools=build_advisor_tools(language),
            execute=execute,
            max_steps=settings.llm.max_agent_steps,
...
async def get_insights(
    ...
    *,
    ...
    serve_cached = not refresh or bool(await cache.get(_insights_cooldown_key(vendor.id, language)))
    if serve_cached:
