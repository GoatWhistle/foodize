...
def _confirmation_error(ctx: OrderToolContext, stored: str | None) -> str | None:
    confirmed_turn, confirm_token, _ = parse_confirm(stored)
    ...
    if confirmed_turn is None or confirmed_turn >= ctx.user_turn:
        return dumps(
            {
                "error": "cart_not_confirmed",
                "message": translate("prompts.order.results.cartNotConfirmedByUser", ctx.language),
            }
        )
    return None
...
async def _validate_confirmed_cart(
    ...
    _, confirm_token, confirmed_hash = parse_confirm(stored)
    if confirm_token is None:
        return dumps({"error": "cart_not_confirmed"}), None, None
...
async def _submit_order(
    ...
    try:
        ...
        if error_code is None:
            raise
        return dumps({"error": error_code, "message": exc.detail})


async def place_order_tool(ctx: OrderToolContext, args: JsonObject) -> str:
    ...
    if validation_error is not None or cart is None or confirm_token is None:
        return validation_error or dumps({"error": "cart_not_confirmed"})

    raw_promo = args.get("promo_code")
    promo_code = raw_promo if isinstance(raw_promo, str) and PROMO_RE.match(raw_promo) else None
