...
async def test_menu_crud(db_session):
    vendor_data = UserCreate(
        name="Vendor3", phone_number="79006666666", password="strongpassword", user_role=UserRole.VENDOR
    )
    ...
    item_create = MenuItemCreate(name="Sushi", description="Fish", price=800, category=Category.SHAURMA)
