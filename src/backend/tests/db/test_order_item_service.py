...
async def test_get_menu_items_by_ids(db_session):
    vendor_data = UserCreate(
        name="Vendor2", phone_number="79005555555", password="strongpassword", user_role=UserRole.VENDOR
    )
    ...
    m1 = MenuItem(
        restaurant_id=restaurant.id, name="Item1", description="desc", price=100, category=Category.SHAURMA.value
    )
    m2 = MenuItem(
        restaurant_id=restaurant.id, name="Item2", description="desc", price=200, category=Category.SHAURMA.value
    )
