...
from features.restaurants.crud import create_restaurant, get_vendor_restaurants
...
async def test_create_restaurant_crud(db_session):
    ...
    vendor_data = VendorCreate(description="Test Vendor")
    vendor_profile = await create_vendor_profile(db_session, user, vendor_data)
