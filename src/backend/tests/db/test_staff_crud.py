...
from features.staff.crud import (
    create_staff_request_in_db,
    get_last_request,
    get_request_by_id,
    get_requests_by_vendor_id,
    get_staff_profile_by_user_id,
    update_request_status_and_create_profile,
)
...
async def vendor_and_restaurant(db_session):
    vendor_data = UserCreate(
        name="Vendor", phone_number="79003333333", password="strongpassword", user_role=UserRole.VENDOR
    )
...
async def staff_candidate(db_session):
    user_data = UserCreate(
        name="Candidate", phone_number="79004444444", password="strongpassword", user_role=UserRole.CUSTOMER
    )
...
async def test_staff_crud_lifecycle(db_session, vendor_and_restaurant, staff_candidate):
    ...
    request = await create_staff_request_in_db(db_session, candidate.id, restaurant.id, data)
    ...
    updated_req = await update_request_status_and_create_profile(
        db_session, request, StaffRequestStatus.ACCEPTED, create_profile=True
    )
...
async def test_update_request_status_no_profile(db_session, vendor_and_restaurant, staff_candidate):
    ...
    updated_req = await update_request_status_and_create_profile(
        db_session, request, StaffRequestStatus.REJECTED, create_profile=False
    )
