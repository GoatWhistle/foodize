...
class TestTelegramCheck:
    ...
    async def test_check_unregistered_user(self, client: AsyncClient):
        ...
        with patch(
            "features.telegram.api.webapp_auth.telegram_check",
...
class TestTelegramSiteLoginRequestCode:
    ...
    async def test_request_code_success(self, client: AsyncClient):
        with patch(
            "features.telegram.api.site_login.request_site_login_code",
            new_callable=AsyncMock,
            return_value=None,
        ):
            response = await client.post(
    ...
    async def test_request_code_by_username_success(self, client: AsyncClient):
        with patch(
            "features.telegram.api.site_login.request_site_login_code_by_username",
            new_callable=AsyncMock,
            return_value=None,
        ):
            response = await client.post(
...
class TestTelegramSiteLoginVerify:
    ...
    async def test_verify_success(self, client: AsyncClient):
        tokens = _make_tokens()
        with patch(
            "features.telegram.api.site_login.verify_site_login_code",
            new_callable=AsyncMock,
            return_value=tokens,
        ):
            response = await client.post(
    ...
    async def test_verify_by_username_success(self, client: AsyncClient):
        tokens = _make_tokens()
        with patch(
            "features.telegram.api.site_login.verify_site_login_code_by_username",
            new_callable=AsyncMock,
            return_value=tokens,
        ):
            response = await client.post(
...
class TestTelegramRegister:
    ...
    async def test_register_new_user(self, client: AsyncClient):
        tokens = _make_tokens()
        with patch(
            "features.telegram.api.webapp_auth.telegram_register",
            new_callable=AsyncMock,
            return_value=tokens,
        ):
            response = await client.post(
...
class TestTelegramAuth:
    ...
    async def test_auth_existing_user(self, client: AsyncClient):
        tokens = _make_tokens()
        with patch(
            "features.telegram.api.webapp_auth.telegram_auth_existing",
            new_callable=AsyncMock,
            return_value=tokens,
        ):
            response = await client.post(
...
class TestTelegramSetPassword:
    ...
    async def test_set_password_as_user(self, client: AsyncClient, as_user):
        ...
        with patch(
            "features.telegram.api.site_login.set_site_password",
            new_callable=AsyncMock,
            return_value=user,
        ):
            response = await client.post(
...
class TestTelegramLogout:
    ...
    async def test_logout_as_user(self, client: AsyncClient, as_user):
        user = _make_user_read()
        with patch(
            "features.telegram.api.webapp_auth.unlink_telegram_for_user",
            new_callable=AsyncMock,
            return_value=user,
        ):
            response = await client.post("/api/v1/telegram/logout")
