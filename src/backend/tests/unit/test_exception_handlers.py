...
class TestAppExceptionHandler:
    ...
    async def test_returns_correct_status_and_detail(self):
        ...
        response = await app_exception_handler(MagicMock(), exc)
...
class TestUnhandledExceptionHandler:
    ...
    async def test_returns_500(self):
        response = await unhandled_exception_handler(MagicMock(), Exception("boom"))
...
class TestIntegrityErrorHandler:
    ...
    async def test_restaurants_address_constraint(self):
        ...
        response = await integrity_error_handler(MagicMock(), exc)
        assert response.status_code == 400
