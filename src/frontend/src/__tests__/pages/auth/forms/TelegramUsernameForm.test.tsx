...
describe('TelegramUsernameForm', () => {
  it('renders username input with disabled submit initially', () => {
    ...
    expect(screen.getByRole('button', { name: 'Получить код в Telegram' })).toBeDisabled();
  });

  it('strips @ and invalid characters from input', async () => {
    const user = userEvent.setup();
    render(<Harness />);
    const input = screen.getByLabelText('Telegram @username');
  ...
  it('shows hint when partial invalid username typed', async () => {
    const user = userEvent.setup();
    render(<Harness />);
    await user.type(screen.getByLabelText('Telegram @username'), 'abc');
    expect(screen.getByText('a–z, 0–9 и _ · минимум 5 символов')).toBeInTheDocument();
  });
  ...
  it('shows underscore error when starting with underscore after touch', async () => {
    ...
    await user.tab();
    expect(screen.getByText('Не может начинаться или заканчиваться на _')).toBeInTheDocument();
  });
  ...
  it('enables submit for valid username and calls onSubmit', async () => {
    ...
    await user.type(screen.getByLabelText('Telegram @username'), 'validuser');
    const submit = screen.getByRole('button', { name: 'Получить код в Telegram' });
  ...
  it('does not call onSubmit when submitting invalid username via form', async () => {
    ...
    const form = screen.getByLabelText('Telegram @username').closest('form') as HTMLFormElement;
    form.requestSubmit();
  ...
  it('shows max-length error when username exceeds 32 chars on blur', async () => {
    ...
    await user.click(screen.getByLabelText('Telegram @username'));
    await user.tab();
    expect(screen.getByText('Максимум 32 символа')).toBeInTheDocument();
  });
