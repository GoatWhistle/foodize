...
describe('authService', () => {
  ...
  it('register sends POST to /register', async () => {
    ...
    const result = await authService.register({ name: 'Ivan' } as never);
    expect(result.data).toEqual(mockData);
  });
  ...
  it('logout sends POST to /logout so backend can invalidate tokens', async () => {
    ...
    const result = (await authService.logout()) as AxiosResponse;
  ...
  it('requestTelegramLoginCode sends POST to telegram site-login', async () => {
    ...
    const result = await authService.requestTelegramLoginCode({ phone: '79001234567' } as never);
  ...
  it('requestTelegramLoginCodeByUsername sends POST', async () => {
    ...
    const result = await authService.requestTelegramLoginCodeByUsername({ username: 'user' } as never);
    expect(result.data).toEqual({ ok: true });
  });

  it('verifyTelegramLoginCode sends POST', async () => {
    mock.onPost('/telegram/site-login/verify').reply(200, { token: 'abc' });
    const result = await authService.verifyTelegramLoginCode({ code: '123456' } as never);
  ...
  it('verifyTelegramLoginCodeByUsername sends POST', async () => {
    ...
    const result = await authService.verifyTelegramLoginCodeByUsername({ username: 'u', code: '1' } as never);
    expect(result.data).toEqual({ token: 'abc' });
  });
