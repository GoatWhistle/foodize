...
const getDateStart = (date) => new Date(date.getFullYear(), date.getMonth(), date.getDate());
          ...
          <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-surface)" }}>
            ...
            {notifications.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {unreadCount > 0 && (
                  <button onClick={markAllAsRead} style={{ background: "none", border: "none", color: "var(--brand)", fontSize: "0.8rem", fontWeight: 700, cursor: "pointer", padding: 0 }}>
                    Прочитать все
                  </button>
                )}
                <button onClick={deleteAll} aria-label="Удалить все" style={{ width: 28, height: 28, borderRadius: "var(--r-xs)", border: "1px solid var(--border)", background: "var(--bg-card)", color: "var(--text-3)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {notifications.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                {hasMore && (
                  <button onClick={handleLoadMore} disabled={loadingMore} style={{ width: "100%", padding: "10px 16px", background: "none", border: "none", borderTop: "1px solid var(--border)", color: "var(--brand)", fontSize: "0.8rem", fontWeight: 700, cursor: loadingMore ? "default" : "pointer", opacity: loadingMore ? 0.6 : 1 }}>
