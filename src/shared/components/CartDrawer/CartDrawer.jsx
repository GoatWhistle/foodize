...
import OrderButton from "@shared/components/OrderButton/OrderButton.jsx";
import { promoService } from "@shared/services/promoService.js";
...
const CartDrawer = ({ onClose, isRestaurantOpen = true, onHaptic }) => {
  ...
  const orderingUnavailable = loadEstimate && loadEstimate.ordering_available === false;
  ...
  const pickupTooSoon = pickupMode === "scheduled" && requestedPickupAt && new Date(requestedPickupAt) < minPickupDate;
      ...
      <div className={s.drawer} ref={drawerRef}>
        ...
        <div className={s.inner}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {selectedOptions.length > 0 && (
                      <div style={{ marginTop: 3, fontSize: "0.72rem", lineHeight: 1.35, color: "var(--text-3)" }}>
                        {selectedOptions.map((o) => `${o.name}${o.price_delta ? ` +${o.price_delta} ₽` : ""}`).join(", ")}
                      </div>
                    )}
                  </div>
                  <div className={s.itemControls}>
                    <button className={s.qtyBtn} onClick={() => removeFromCart(menuItem.id, selectedOptionIds)} aria-label="Уменьшить">
                      <Minus size={12} weight="bold" />
                    </button>
                    <span style={{ fontWeight: 700, minWidth: 20, textAlign: "center", fontSize: "0.9rem" }}>{quantity}</span>
                    <button className={s.qtyBtn} onClick={() => addToCart(menuItem, cartRestaurantId, selectedOptions)} aria-label="Увеличить">
          ...
          {!appliedPromo ? (
            ...
            <div style={{ marginTop: 16, padding: "10px 14px", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "var(--r-md)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.85rem", color: "#22c55e", fontWeight: 700 }}>
                <Tag size={14} weight="fill" />
                {appliedPromo.code}
                {appliedPromo.discount_type === "PERCENT" ? ` −${appliedPromo.discount_value}%` : ` −${appliedPromo.discount_value} ₽`}
              </div>
              <button onClick={handleRemovePromo} style={{ background: "none", border: "none", cursor: "pointer", color: "#22c55e", display: "flex" }}>
          ...
          <div className={s.total} style={{ marginTop: 16 }}>
            ...
            <span className={s.totalValue} style={appliedPromo ? { color: "#22c55e" } : undefined}>{finalTotal} ₽</span>
          </div>
          ...
          {loadEstimate && (
            <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: "var(--r-md)", border: orderingUnavailable ? "1px solid var(--error)" : hasQueueWarning ? "1px solid #f59e0b" : "1px solid var(--border)", background: orderingUnavailable ? "rgba(239,68,68,0.1)" : hasQueueWarning ? "rgba(245,158,11,0.12)" : "var(--bg-card)", display: "flex", gap: 10, alignItems: "flex-start" }}>
              {orderingUnavailable ? (
                <WarningCircle size={18} weight="fill" color="var(--error)" />
              ) : (
                <Clock size={18} weight="fill" color={hasQueueWarning ? "#f59e0b" : "var(--text-3)"} />
              )}
              <div style={{ flex: 1 }}>
