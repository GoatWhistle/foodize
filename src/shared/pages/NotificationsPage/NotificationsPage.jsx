...
const groupByDay = (notifications) => {
  ...
  for (const n of notifications) {
    const day = getDateStart(new Date(n.created_at));
      ...
      {loading ? (
        ...
        <>
          {groups.map(({ label, items }) => (
            <div key={label}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.05em", padding: "8px 16px 4px" }}>
                {label}
              </div>
              {items.map((n) => (
                <div
          ...
          {hasMore && (
            <div style={{ padding: "12px 16px" }}>
              <button className="btn btn-secondary" style={{ width: "100%" }} disabled={loadingMore} onClick={handleLoadMore}>
