...
import { CaretRight, Check, FileText, Shield } from "@phosphor-icons/react";
    ...
    <div className={`${s.page}${pageClassName ? ` ${pageClassName}` : ""}`}>
      ...
      <section className={s.section}>
        ...
        <div className={s.card}>
          ...
          {editSuccess && <div className={s.success}><Check size={14} weight="bold" /> Данные сохранены</div>}
      ...
      {showPasswordChange && (
        <section className={s.section}>
          ...
          <form className={s.card} onSubmit={(e) => { void handlePasswordChange(e); }}>
            ...
            {pwSuccess && <div className={s.success}><Check size={14} weight="bold" /> Пароль изменён</div>}
