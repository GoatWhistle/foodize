const ERROR_MAP = {
  ...
  "You can publish up to 5 reviews for one restaurant": "Можно опубликовать до 5 отзывов на один ресторан",
  "Duplicate entry: this information already exists": "Вы уже оставили отзыв на этот ресторан",
  "You can only review restaurants where you have a completed order": "Отзыв можно оставить только при наличии завершённого заказа",
  "You already have a pending request for this restaurant.": "У вас уже есть активная заявка на этот ресторан",
  "Your previous request was rejected. Please try again after 24 hours.": "Ваша предыдущая заявка была отклонена. Повторная подача возможна через 24 часа",
  "You are already a staff member at this restaurant.": "Вы уже являетесь сотрудником этого ресторана",
  ...
  "One or more menu items do not belong to the specified restaurant": "Один или несколько товаров не принадлежат этому ресторану",
  "Order can only be cancelled when in PENDING or ACCEPTED status": "Отменить заказ можно только пока он ожидает или готовится",
  "Invalid order status transition": "Недопустимое изменение статуса заказа",
  "One or more menu items are not available": "Один или несколько товаров недоступны для заказа",
  "Order can only be completed when in READY status": "Подтвердить получение можно только в статусе «Готов»",
  ...
  "Restaurant is temporarily not accepting orders": "Заведение временно поставило приём заказов на паузу",
  "Duplicate options selected": "Одна и та же опция выбрана дважды",
  "Selected option not found": "Одна из выбранных опций больше недоступна. Обновите меню и попробуйте снова",
  "Selected option does not belong to menu item": "Одна из выбранных опций не относится к этому блюду",
  ...
  "Promo code is not valid for this restaurant": "Промокод не действителен для этого ресторана",
...
export function translateApiError(err: unknown, fallback?: string): string {
  ...
  const statusFallbacks = HTTP_STATUS_FALLBACKS as Record<number, string>;

  if (detail && typeof detail === "string") {
    const errorMap = ERROR_MAP as Record<string, string>;
  ...
  if (status != null && statusFallbacks[status]) {
    return fallback ?? statusFallbacks[status];
  }
