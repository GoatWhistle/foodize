...
from aiogram.types import Message, User, Chat, ReplyKeyboardMarkup, InlineKeyboardMarkup, Contact
...
def test_vendor_status_text():
    ...
    assert "Причина: test" in _vendor_status_text({"is_vendor": True, "approval_status": "REJECTED", "rejection_reason": "test"})
...
async def test_link_phone_failure_no_secret():
    ...
    assert res is False
    m.answer.assert_called_with("Бот пока не настроен для регистрации: не задан TELEGRAM__BOT_API_SECRET.")
...
async def test_link_phone_http_status_errors(mocker):
    ...
    res = await _link_phone(m, "+79990000000")
    assert res is False
    m.answer.assert_called_with("Не получилось привязать телефон. Проверьте номер и попробуйте еще раз.")
...
async def test_cmd_vendor_status(mocker):
    ...
    mock_post.return_value = httpx.Response(200, request=req, json={"data": {"is_vendor": True, "approval_status": "APPROVED"}})
    await cmd_vendor_status(m)
...
async def test_cmd_orders(mocker):
    ...
    mock_post.return_value = httpx.Response(200, request=req, json={"data": [{"display_id": "123", "restaurant_name": "Cafe", "status": "COOKING", "total_price": 1000}]})
    await cmd_orders(m)
...
async def test_cmd_start_deep_link_restaurant(mocker):
    ...
    mock_get.return_value = httpx.Response(200, request=req, json={"data": {"name": "Cafe Delicious"}})
